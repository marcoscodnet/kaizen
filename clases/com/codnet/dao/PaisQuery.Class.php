...
class PaisQuery {
	...
	static function getPaisPorId(Pais $obj) {
		...
		$sql = "SELECT cd_pais, ds_pais FROM pais WHERE cd_pais = $cd_pais";
	...
	static function modificarPais(Pais $obj) {
		...
		$cd_pais = $obj->getCd_pais();
		$ds_pais = $obj->getDs_pais();
		$sql  = "UPDATE pais SET cd_pais=$cd_pais, ds_pais='$ds_pais' ";
	...
	static function eliminarPais(Pais $obj) {
		$db = DbManager::getConnection();
		$cd_pais = $obj->getCd_pais ();
		$sql = "DELETE FROM pais WHERE cd_pais = $cd_pais";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
		throw new DBException($db->sql_error());
	}
