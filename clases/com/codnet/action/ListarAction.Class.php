...
/**
 * Acci�n listar entidades gen�ricas.
 * Cada subclase definir� la entidad concreta. 
 * 
 * @author bernardo
 * @since 05-03-2010
 * 
...
abstract class ListarAction extends SecureOutputAction{
	...
	protected function getXTemplate(){
		...
		if( defined("DEFAULT_LISTAR_TEMPLATE") )
			...
			return new XTemplate(APP_PATH. CLASS_PATH . 'codnet/view/listar_template.html');
	...
	protected function getMensajeError(){
		...
		if (isset ( $_GET ['msg'] ))
			...
			$msg = "No se pudo realizar la operaci�n para el elemento con ID <b>$cd</b>.";
	...
	protected function getUrlPaginador( $orden , $campoFiltro, $filtro, $campoOrden ){
		$url = 'doAction?action='. $this->getUrlAccionListar() . '&orden=' . $orden . '&campoFiltro=' . $campoFiltro . '&filtro=' . $filtro. '&campoOrden=' . $campoOrden;
		return $url;
	}
	
	private function getParam($name, $default=''){
		if (isset ( $_GET [$name] ))
		$value = $_GET [$name]; else
		$value = $default;
		return $value;
	}
	...
	*/
	 ...
	 * @return none.
	 */
	protected function parseAccion(XTemplate $xtpl, $onclick, $href, $img, $title){
